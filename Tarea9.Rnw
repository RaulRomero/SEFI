...
@
Retorno del benchmark: \Sexpr{sinteticos[2]}. Conclusiones del método:

\begin{enumerate}
\item Después de igualar las volatilidades y crear un portafolio sintético haciendo uso de CETES con 3\% anual, el fondo \Sexpr{n_acciones[1]} tiene \Sexpr{sinteticos[1]} por ciento de retorno, por lo que se puede afirmar que convenía en el periodo analizado invertir en el fondo en lugar del mercado. 
\end{enumerate}

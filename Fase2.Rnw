...
Clasificación del Fondo: Renta Variable
...
Informaci\'on Administrativa: Manuel De Jesús Somoza Alonso. Administrador principal desde may 8, 2009
...
\subsection{Presentación matemática}
...
A continuación se obtiene el rendimiento con los métodos ya mencionados. Los resultados se muestran abajo:
...
Debido a los números obtenidos, se concluye que los fondos a utilizarse serán:
\begin{enumerate}
\item FRANOPRB1.MX, por ser 8 veces de los tres más destacados en las características mencionadas y visualmente notables en la tabla anterior.
\item GBMMODBO.MX, por ser 8 veces de los tres más destacados en las características mencionadas y visualmente notables en la tabla anterior.
\item ACTI500B3.MX, por ser 7 veces de los tres más destacados en las características mencionadas y visualmente notables en la tabla anterior.
\end{enumerate}

Cabe mencionar que HSBCBOLBFV.MX y \Sexpr{nacciones[1]} fueron solo una vez parte de los 3 mejores en las características anteriores.
...
Es una medida de exceso de rendimiento por unidad de riesgo de una inversión.
...
En este modelo el retorno adicional del portafolio es ajustado en base al risego que tiene en comparación al Benchmark.  M2 nos indica que tan bueno es el nivel de recompensa en relación al riesgo tomado.
...
m2 <- m2*100
@
Retorno del Benchmark: \Sexpr{as.numeric(rend_anual[length(rend_anual)])} por ciento.Conclusiones del método: 
\begin{enumerate}
...
Al igual que el M2, en retorno diferencial, también se trata de una medida ajustada del riesgo. En este caso es el retorno del mercado (benchmark) el que es ajustado al riesgo del portafolio. 
...
DR <- DR*100
@
Retorno del Benchmark: \Sexpr{as.numeric(rend_anual[length(rend_anual)])}. Conclusiones del método:
\begin{enumerate}
...
Se muestran los resultados de todos los fondos, y a continuación el análisis de los tres elegidos.
En el fondo \Sexpr{nacciones[2]} se obtuvo un ratio Sharpe de \Sexpr{sharpe[2]} lo que indica un rendimiento inferior al de rentabilidad sin riesgo, en cuanto a el retorno M2 \Sexpr{m2[2]}  lo que indica que el rendimiento del activo es inferior al riesgo que se está asumiendo. Ahora observando el retorno diferencial \Sexpr{DR[2]} nos presenta  que bajo la misma volatilidad el mercado hubiera tenido un mejor comportamiento que el portafolio o fondo.  Con el análisis de GH1 obtenido de \Sexpr{indicador[2]} nos indica que bajo las mismas condiciones de volatilidad un portafolio sintético lo hubiera hecho un poco mejor. Finalmente mediante GH2 \Sexpr{gh2[2]} nos indica que el portafolio tuvo un mejor rendimiento que el Benchmark, asumiendo las mismas condicione de riesgo.
En el fondo \Sexpr{nacciones[4]} se obtuvo un ratio Sharpe de \Sexpr{sharpe[4]} lo que indica un rendimiento inferior al de rentabilidad sin riesgo, en cuanto al retorno M2 \Sexpr{m2[4]} lo que indica que es mejor la rentabilidad del fondo ajustada al riesgo tomado. Ahora observando el retorno diferencial \Sexpr{DR[4]} hay compensación en el fondo en relación al Benchmark. Con el análisis de GH1 obtenido de \Sexpr{indicador[4]} nos indica que bajo las mismas condiciones de volatilidad un portafolio sintético lo hubiera hecho un poco mejor. Finalmente mediante GH2 \Sexpr{gh2[4]} nos indica que el portafolio tuvo un mejor rendimiento que el Benchmark, asumiendo las mismas condicione de riesgo.
En el fondo \Sexpr{nacciones[5]} se obtuvo un ratio Sharpe de \Sexpr{sharpe[5]} lo que indica un rendimiento inferior al de rentabilidad sin riesgo, en cuanto al retorno M2 \Sexpr{m2[5]} esto indica que es mejor la rentabilidad del fondo ajustada al riesgo tomado. Ahora observando el retorno diferencial \Sexpr{DR[5]} hay compensación en el fondo en relación al Benchmark. Con el análisis de GH1 obtenido de \Sexpr{indicador[5]} nos indica que bajo las mismas condiciones de volatilidad un portafolio sintético lo hubiera hecho un poco mejor. Finalmente mediante GH2 \Sexpr{gh2[5]} nos indica que el portafolio tuvo un mejor rendimiento que el Benchmark, asumiendo las mismas condicione de riesgo.

\section{Regresión}
El motivo de realizar una regresión es la obtención de información al momento de graficar los retornos del fondo contra los del \emph{benchmark} en un diagrama de dispersión conociendo la relación entre ellos. La regresión tiene la siguiente forma:
...
\begin{itemize}
  ...
  \item $\alpha_R$ = Alfa de la regresión
  \item $\beta_R$ = Beta de la regresión
  \item $\epsilon_R$ = Epsilon de la regresión (error)
  \item b = Retorno del benchmark

\end{itemize}

\subsection{CAPM}
Adaptando lo escrito anteriormente en la ecuación \ref{jotostodos} al modelo CAPM encontramos lo siguiente:
...
\begin{itemize}
  ...
  \item $\beta$ = Medida de Riesgo sistemático. Pendiente de retornos del Benchmark
(X) contra retornos del stock (Y).

\end{itemize}

Terminando entonces con el modelo CAPM a partir de la regresión:
...
Es una medida de riesgo ajustado a la tasa libre de riesgo que nos permite definir si un portafolio obtiene un rendimiento apropiado dado su nivel de riesgo. Cabe mencionar que si nos enfocamos en el modelo CAPM $\alpha$ debería de ser cero. Un número mayor a cero nos indica que el portafolio obtuvo retornos adicionales.  Uno menor a cero nos indica que el fondo obtuvo retornos devaluados. Es comúnmente utilizado como  \emph{valor agregado} al retorno por parte del administrador del fondo.
...
Es un coeficiente de la volatilidad  del mercado dentro de nuestro portafolio y es relacionado directamente con el riesgo sistemático. El coeficiente como tal indica l proporción de riesgo/rendimiento del mercado que se encuentra en nuestro portafolio.
...
\subsubsection{Cálculo de CAPM, $\alpha$ y de $\beta$ para los tres fondos}
...
\item HSBC. (2013). Fondos de inversión [Internet]. México. Disponible desde <https://www.hsbc.com.mx/1/2/es/personas/\\ inversiones-fondos/fondos-de-inversion/deuda-local-premier?Wt.ac=HSBC-BOL> [Acceso 15 de octubre de 2015]. 
...
\item FRANKLIN U.S. OPPORTUNITIES FUND, 2015. DOCUMENTO CON INFORMACIÓN CLAVE PARA LA INVERSIÓN. [Online]
